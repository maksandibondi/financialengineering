...
% Simulation features.
NT = 180;                % Number of time increments.
...
E = exp((r-q-0.5*v^2)*dt + sqrt(dt)*v.*randn(NS,NT-1));
All = [FirstCol E];
S = cumprod(All,2);
clear FirstCol E All
...
[EuroPriceLSM AmerPriceLSM] = BlackScholesLSM(S,K,r,q,T,NS,NT,dt,PutCall,XmatrixHandle);
