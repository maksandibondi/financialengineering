...
for k = 1:discretization_num_t
    for i = 1:discretization_num_S
        [priceC_Merton(k,i),priceP_Merton(k,i)] = PriceEU_MC_Merton(S(i),K,r,sigma,m_y,sigma_y,sigma_j,jump_freq,t(k));
        [priceC_jumpsonly(k,i),priceP_jumpsonly(k,i)] = PriceEU_MC_jumpsonly(S(i),K,r,jump_height,jump_freq,t(k));
        [priceC(k,i),priceP(k,i)] = PriceEU_MC(S(i),K,r,sigma,t(k));
    end;
end;

%% Final condition
plot(S,priceC_Merton(end,:),'black');
hold on;
plot(S,priceP_Merton(end,:),'black');
plot(S,priceC_jumpsonly(end,:),'r');
plot(S,priceP_jumpsonly(end,:),'r');
plot(S,priceC(end,:),'blue');
plot(S,priceP(end,:),'blue');

%% Surface
figure;
surf(S,t,priceC_Merton);
figure;
surf(S,t,priceP_Merton);
