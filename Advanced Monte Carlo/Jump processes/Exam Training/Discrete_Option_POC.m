...
for k = 1:discretization_num_t
    for i = 1:discretization_num_S
        [priceC_Discrete(k,i),priceP_Discrete(k,i)] = PriceEU_MC_discrete(S(i),K,r,sigma,jump_freq,t(k),T,discretization_num_t,v1,p1,v2,p2);
        [priceC(k,i),priceP(k,i)] = PriceEU_MC(S(i),K,r,sigma,t(k));
    end;
end;

%% Final condition
plot(S,priceC_Discrete(end,:),'black');
hold on;
plot(S,priceP_Discrete(end,:),'black');
plot(S,priceC(end,:),'blue');
plot(S,priceP(end,:),'blue');

%% Surface
figure;
surf(S,t,priceC_Discrete);
figure;
surf(S,t,priceP_Discrete);
