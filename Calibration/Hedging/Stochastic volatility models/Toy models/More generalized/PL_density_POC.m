...
for k = 1:s
    ...
    for i = 1:num_of_iter
        ...
        [V_call, V_put, P_call, P_put, A_call, A_put, B_call, B_put, PLcall(i,k), PLput(i,k)] = hedging_exact_repl(t0, T, discretization_num_t, r, sigma1, K, S0, hedge_freq(k));
        [V_call_const, V_put_const, P_call_const, P_put_const, A_call_const, A_put_const, B_call_const, B_put_const, PLcall_const(i,k), PLput_const(i,k)] = hedging_exact_repl(t0, T, discretization_num_t, r, sigma, K, S0, hedge_freq(k));
    end;
...
for k = 1:s
    ...
    EV_PLcall_const(k) = sum(PLcall_const(:,k))/num_of_iter;
    Var_PLcall_const(k) = sum(PLcall_const(:,k).^2)/num_of_iter - EV_PLcall_const(k)^2;
end;

display(EV_PLcall);
display(Var_PLcall);
display(EV_PLcall_const);
display(Var_PLcall_const);
    ...
    for i = 1:sz
        ...
        for j = 1:num_of_iter
            ...
            if (i ~= sz)
                ...
                if (PLcall(j,k) < set_of_PL(i+1)) && (PLcall(j,k) >= set_of_PL(i))
                    counter_density = counter_density+1;
                end;
                
                if (PLcall_const(j,k) < set_of_PL(i+1)) && (PLcall_const(j,k) >= set_of_PL(i))
                    counter_density_const = counter_density_const+1;
                end;
                
                if (PLcall(j,k) < set_of_PL(i+1))
                    counter_proba = counter_proba+1;
                end;
                
                if (PLcall_const(j,k) < set_of_PL(i+1))
