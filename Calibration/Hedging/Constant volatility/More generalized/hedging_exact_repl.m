...
%% Main
V0_call = BSTheory_call(S0,t_0,T,K,r,sigma);
V0_put = BSTheory_put(S0,t_0,T,K,r,sigma);
V_call(1) = V0_call;
V_put(1) = V0_put;

S(1) = S0;
A_call(1) = normal(d1(S0,t_0,T,K,r,sigma)); % delta
B_call(1) = V_call(1)-S(1)*A_call(1);
P_call(1) = A_call(1)*S(1) + B_call(1);

A_put(1) = normal(d1(S0,t_0,T,K,r,sigma))-1; % delta
B_put(1) = V_put(1)-S(1)*A_put(1);
...
for i = 2:discretization_num_t
    ...
    if ismember(i,hedge_array)
        ...
        A_call(i) = normal(d1(S(i),t(i),T,K,r,sigma)); % New alpha recalculation
        A_put(i) = normal(d1(S(i),t(i),T,K,r,sigma))-1; % New alpha recalculation
    ...
    V_call(i) = BSTheory_call(S(i),t(i),T,K,r,sigma); 
    V_put(i) = BSTheory_put(S(i),t(i),T,K,r,sigma);
