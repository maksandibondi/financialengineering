...
for k = 1:size(inputStructure.T_normalized, 2)
    LVATM_ = LVATM(k,:);
    VolImpATM_ = sort(VolImpATM(k,:));
    idxnan = isnan(LVATM_); %% find indices with non NAN values
    [prmReg, rsquared] = polynomialFitting(VolImpATM_(~idxnan),LVATM_(~idxnan),1);
    [prmReg2, rsquared2] = polynomialFitting(VolImpATM_(~idxnan),LVATM_(~idxnan),3);
    paramReg(k,:) = prmReg;
    paramReg2(k,:) = prmReg2;
    rsq(k) = rsquared;
    rsq2(k) = rsquared2;
    LVAMT_model = polyval(paramReg(k,:),VolImpATM_(~idxnan)); %% values obtained by model
    LVAMT_model2 = polyval(paramReg2(k,:),VolImpATM_(~idxnan)); %% values obtained by model    

    %% visualize regression results
    f(k) = figure; f(k).Visible = 'off'; f(k).Tag = 'ATMonImplied reg';
    s1 = scatter(VolImpATM_(~idxnan),LVATM_(~idxnan));
    hold on
    p1 = plot(VolImpATM_(~idxnan),LVAMT_model); xlabel('impvol ATM'); ylabel('LVATM');
    p2 = plot(VolImpATM_(~idxnan),LVAMT_model2); xlabel('impvol ATM'); ylabel('LVATM');
    ttl = sprintf('Linear regressions of ATM Local Vol(t) on Implied vol(t) with fixed T=%s',num2str(inputStructure.T_normalized(k)));
    title(ttl);
    legend([s1;p1;p2], 'LV ATM real', 'LV ATM lin regr order1', 'LV ATM lin regr order3');
end;
% Conclusion : sometimes negative coef of determination, bad fitting
...
for k = 1:size(inputStructure.T_normalized, 2)
    for i = 1:size(validationDates,2)
        LVATM_model_vld(k,i) = polyval(paramReg(k,:), VolImpATM_vld(k,i));
        LVATM_model_vld2(k,i) = polyval(paramReg2(k,:), VolImpATM_vld(k,i));
    end;
end;
