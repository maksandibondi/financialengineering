...
%   Detailed explanation goes here
    ...
    for i = 1:disc_T 
        localVolCalibratedNormalizedScale_temp(i,:) = interp1(K, localVolCalibrated(i,:), K_normalized, interpMethod);
    end;
    
     for j = 1:size(localVolCalibratedNormalizedScale_temp,2) 
        localVolCalibratedNormalizedScale(:,j) = interp1(T, localVolCalibrated(:,j), T_normalized, interpMethod);
    end;
